function odata = encrypt_encode(idata,key)
...
%   Detailed explanation goes here
    ...
    for x=1:512:length(idata)
        ...
        M=mod(key*M*key+key,256);
        for xx=1:8
            for yy=1:8
                 M(xx,yy)=bitshift(s(bitshift(M(xx,yy),-4)+1),4)+s(bitand(M(xx,yy),15)+1);
            end;
        end;
        %M=mod(key*bin2dec(char(idata(x+temp_M*8:x+temp_M*8+7)+48))*key+key,256);
        %M=bitshift(s(bitshift(M,-4)),4)+s(bitand(M,15))
        M=mod(key*M*key+key,256);
        for list=0:63
